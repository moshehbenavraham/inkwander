...
const About = () => {
  ...
  useSEO({
    ...
    description:
      "Perspective is an editorial blog publishing thoughtful essays on wellness, travel, creativity, and personal growth. Learn about our mission and values.",
    path: "/about",
  });
  ...
  const handleSubscribe = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const result = await subscribeNewsletter({
      email: trimmed,
      source: "About page community section",
    });
    setSubmitting(false);
  ...
  return (
    <div className="min-h-screen bg-background animate-fade-in">
      ...
      <main id="main" className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        {/* Hero Section */}
        ...
        <section className="text-center py-12 rounded-2xl bg-card">
          ...
          <p className="text-muted-foreground mb-8 max-w-2xl mx-auto px-6">
            Subscribe to receive our latest articles, insights, and inspiration directly in your inbox.
          </p>
          <form
            onSubmit={handleSubscribe}
            noValidate
            className="flex flex-col sm:flex-row gap-4 max-w-md mx-auto px-6"
            aria-label="Newsletter signup"
            aria-describedby="about-newsletter-status"
          >
          ...
          <p
            id="about-newsletter-status"
            role={feedback?.tone === "error" ? "alert" : "status"}
            aria-live={feedback?.tone === "error" ? "assertive" : "polite"}
            className={
              feedback
                ? feedback.tone === "error"
                  ? "text-sm text-destructive max-w-md mx-auto mt-4 px-6"
                  : "text-sm text-muted-foreground max-w-md mx-auto mt-4 px-6"
                : "sr-only"
            }
          >
            {feedback?.message ?? ""}
          ...
          {!HAS_NEWSLETTER_ENDPOINT && !feedback && (
            <p className="text-xs text-muted-foreground max-w-md mx-auto mt-3 px-6">
